...
class WebSocket_Handler:
    ...
    async def _create_ws_connection(self):
        ...
        self.ws = await self._session.ws_connect(
            f"{self.gateway_url}/?v=10&encoding=json&compress=zlib-stream", max_msg_size=0
        ) if self.decompresser else await self._session.ws_connect(
                        ...
                        if self._command_tree:
                            ...
                            if command_data:
                                ...
                                if prov_args_count_len in range(req_arg_count, max_arg_count + 1):
                                    await self._trigger(
                                        command_data["coro"],
                                        handler_base(session=self._session, auth=self._auth, data_tree=data["d"]),
                                        *args
                                    )
                                else:
