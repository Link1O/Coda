    ...
    def bind_handlers(shard):
        ...
        async def on_ready_event():
            ...
            await shard.change_presence(
                ...
                value=f"shard no: {shard.shard_id}"
            )
        @shard.on_poll_end
        ...
        async def send_button(ctx: Interaction):
            row = ActionRow(components=[
                Button(label="Click Me!", custom_id="click_me", style=ButtonStyle.SUCCESS)
            ])
        ...
        async def on_button_click(ctx: Interaction):
            await ctx.respond(content=f"You clicked the button! Message context: {ctx.message.content if ctx.message else 'No message content'}", ephemeral=True)
        ...
        async def send_select(ctx: Interaction):
            row = ActionRow(components=[
                StringSelect(
                    ...
                    options=[
                        SelectOption(label="Red", value="red", description="The color of passion"),
                        SelectOption(label="Green", value="green", description="The color of nature"),
                        SelectOption(label="Blue", value="blue", description="The color of the sky")
                    ]
                )
            ])
        ...
        async def send_role_select(ctx: Interaction):
            row = ActionRow(components=[
                RoleSelect(custom_id="select_role", placeholder="Select a role to ping")
            ])
        ...
        async def on_role_select(ctx: Interaction):
            await ctx.respond(content=f"You selected role ID: {ctx.values[0]}", ephemeral=True)
            ...
            await ctx.modal_response(
                ...
                components=[
                    ActionRow(components=[
                        TextInput(custom_id="name", label="What is your name?", placeholder="Pizza steve")
                    ]),
                    ActionRow(components=[
                        TextInput(custom_id="bio", label="Tell us more", style=TextInputStyle.PARAGRAPH, placeholder="Im the coolest pizza slice ever!")
                    ])
                ]
            )
